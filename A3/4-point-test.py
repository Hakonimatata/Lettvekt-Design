...
def sb3p(modelname, L, dL, b, hc, ts1, ts2, ezL, ezT, ezs1, ezs2, ezc, U3, mcore, mcfrp):
    ...
    id = prt.DatumPlaneByPrincipalPlane(principalPlane=YZPLANE, offset=-L/6.0).id
    prt.PartitionCellByDatumPlane(datumPlane=prt.datums[id], cells=prt.cells)
    ...
    edgesLoading1 = prt.sets['EDGES-TRAN'].edges.getByBoundingBox(xMin=-L/6.0, xMax=-L/6.0, zMin=ts1+hc+ts2)
    prt.Set(name='EDGES-LOADING-1', edges=edgesLoading1)
    edgesLoading2 = prt.sets['EDGES-TRAN'].edges.getByBoundingBox(xMin=L/6.0, xMax=L/6.0, zMin=ts1+hc+ts2)
    prt.Set(name='EDGES-LOADING-2', edges=edgesLoading2)
    
    vertsRBX1 = prt.vertices.findAt(((-L/6.0, -b/2, 0.0), ), ((-L/6.0, b/2, 0.0), ))
    prt.Set(name='VERTICES-RBX-1', vertices=vertsRBX1)
    vertsRBY1 = prt.vertices.findAt(((-L/6.0, 0.0, 0.0),))
    prt.Set(name='VERTICES-RBY-1', vertices=vertsRBY1)

    vertsRBX2 = prt.vertices.findAt(((L/6.0, -b/2, 0.0), ), ((L/6.0, b/2, 0.0), ))
    prt.Set(name='VERTICES-RBX-2', vertices=vertsRBX2)
    vertsRBY2 = prt.vertices.findAt(((L/6.0, 0.0, 0.0),))
    prt.Set(name='VERTICES-RBY-2', vertices=vertsRBY2)
    ...
    edges = ins.sets['EDGES-LOADING-1']
    mod.DisplacementBC(name='BC-LOADING-1', createStepName='Initial', region=edges, u3=SET)
    edges = ins.sets['EDGES-LOADING-2']
    mod.DisplacementBC(name='BC-LOADING-2', createStepName='Initial', region=edges, u3=SET)
    
    verts = ins.sets['VERTICES-RBX-1']
    mod.DisplacementBC(name='BC-RBX-1', createStepName='Initial', region=verts, u1=SET)
    verts = ins.sets['VERTICES-RBY-1']
    mod.DisplacementBC(name='BC-RBY-1', createStepName='Initial', region=verts, u2=SET)

    verts = ins.sets['VERTICES-RBX-2']
    mod.DisplacementBC(name='BC-RBX-2', createStepName='Initial', region=verts, u1=SET)
    verts = ins.sets['VERTICES-RBY-2']
    mod.DisplacementBC(name='BC-RBY-2', createStepName='Initial', region=verts, u2=SET)      
     

    mod.StaticStep(name='Step-1', previous='Initial')
    mod.boundaryConditions['BC-LOADING-1'].setValuesInStep(stepName='Step-1', u3=-U3)
    mod.boundaryConditions['BC-LOADING-2'].setValuesInStep(stepName='Step-1', u3=-U3)
